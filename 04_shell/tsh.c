...
void safe_printf(const char *string, ...);
...
void eval(char *cmdline) {
    ...
    if (!builtin_cmd(argv)) {
        ...
        Sigprocmask(SIG_SETMASK, &prev_one, NULL);
        if (status == FG) { /* Parent waits for foreground job */
            int status;
            waitfg(pid);
        } else if (status == BG) {
            /* Output form:
...
void do_bgfg(char **argv) {
    ...
    /* Evaluate PID by the second argument */
    if ((pid = atoi(argv[1]))) { /* PID form */
...
void sigchld_handler(int sig) {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        ...
        if (WIFEXITED(status)) {
            ...
            safe_printf("Job [%d] (%d) terminated by signal 2\n", pid2jid(pid),
                        pid);
            deletejob(jobs, pid);
        } else if (WIFSTOPPED(status)) {
            safe_printf("Job [%d] (%d) stopped by signal 20\n", pid2jid(pid),
                        pid);
...
void safe_printf(const char *string, ...) {
    ...
    while (*ch != '\0') {
        if (*ch == '%') {
            ...
            switch (*ch) {
                case 'd':
                    write_int(STDOUT_FILENO, va_arg(args, int));
                    break;
                case 's':
                    write_str(STDOUT_FILENO, va_arg(args, char *));
                    break;
                case 'c':
